...
func (s *storage) SaveProject(url string, ignoreRobotsTxt, followNofollow, includeNoindex bool, userId int) {
}
...
func TestSaveProject(t *testing.T) {
	// Valid URL
	err := service.SaveProject(projectURL, true, true, true, guid)
	if err != nil {
		t.Error("TestSaveProject: should not return error")
	}

	// Not valid URL
	err = service.SaveProject("....", true, true, true, guid)
	if err == nil {
		t.Error("TestSaveProject: invalid URL should return error")
	}

	// Not supported scheme
	err = service.SaveProject("ftp://example.org", true, true, true, guid)
	if err == nil {
