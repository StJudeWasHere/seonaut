...
func (sr *SqlReporter) HreflangToRedirect(c *models.Crawl) *report_manager.MultipageIssueReporter {
	query := `
		SELECT
			id
	...
	return &report_manager.MultipageIssueReporter{
		Pstream:   sr.pageReportsQuery(query, c.Id),
		ErrorType: reporter_errors.ErrorHreflangToRedirect,
	}
}

// Creates a MultipageIssueReporter object that contains the SQL query to check for pages
// that have hreflang links pointing to error pages with status code 40x or 50x.
func (sr *SqlReporter) HreflangToError(c *models.Crawl) *report_manager.MultipageIssueReporter {
	query := `
		SELECT
			id
		FROM pagereports AS pr
